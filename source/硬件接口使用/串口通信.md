...
对应的设备为ttyS0，完整的串口号为`/dev/ttyS0`

## 2.硬件连接

此处使用Viobot2与windows电脑的串口调试助手作为例子。我们先拿出一个市面上常用的串口转usb模块连接好Viobot2和我们自己的电脑。GND连接好，Viobot2的RX接模块的TX，Viobot2的TX接模块的RX。这里以带扩展版的viobot2为示例。
