...
This section introduces how to use the I2C interface on Viobot2.
...
I2C bus: `IIC6` (Linux device: `/dev/i2c-6`)

## 2. Hardware Connection

We use an MPU9250 module as an example; the same approach applies to other I2C devices.

After powering the module, connect Viobot2 `SCL/SDA` to the MPU9250 `SCL/SDA`.

## 3. Build/Test Example

Linux provides a built-in I2C userspace interface (`linux-i2c`) that can read/write I2C devices directly.

Since the example device is MPU9250, the register addresses used in the program are specific to that chip.

The demo consists of three files and mainly demonstrates basic `write()` and `read()` usage from `<linux/i2c.h>` and `<linux/i2c-dev.h>`.

### (1) `mpu9250.h`

Defines MPU9250 register addresses and declares the `MPU9250` class:
...
Implementation of the `MPU9250` class, including initialization and reading raw accel/gyro and temperature data:
...
This file adds IMU unit conversion and ROS topic publishing. For ROS2 you can adapt the publishing code accordingly. If you do not use ROS, you can also just print the results.
...
This demo continuously reads MPU9250 data and prints it to the terminal.
