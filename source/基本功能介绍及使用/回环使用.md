...
UI上点 设置 选到 loop 选项卡
勾选“开启回环/重定位”，注意不要勾选“加载地图”

![](image/image_Tjs85b0vZQ.png)

### 2.记录词袋

注意“加载地图”的勾不要勾上，勾上表示加载之前的词袋地图。

注意将重定位文件路径 设置为自己想要保存的路径（设备上，不是用户电脑）

因为需要识别关键帧，所以词袋记录是自动的。需要勾选“开启回环/重定位”，点击确定，再启动算法，就会在算法运行的过程中自动记录关键帧。

![](image/image_cAoJa--XiV.png)

当做完了需要录制词袋地图的环境后，点击Viobot-UI的“保存BOW”就可以把词袋地图保存再指定的路径下了。
...
启动算法，会自动加载保存在 `重定位文件路径` 的词袋地图，当新的关键帧能够匹配上词袋里面的关键帧时，则会触发回环。
...
```c++
ros2 action send_goal /baton/loop/keyframe_action loop_action/KeyFrameHandle "{function: 2}"
```
