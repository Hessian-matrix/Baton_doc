...
这里的情形是一台ubuntu系统的ntp客户端设备通过usb接入了viobot2，这个设备已经使用了固定ip：192.168.1.10进行上网，故而在服务器viobot2上配置usb网卡，viobot2能通过usb网络连接到客户端设备。
...
本节主要就是演示了viobot2作为ntp服务器去给局域网中无法连接互联网的设备去做时间同步，做好了时间同步能减少很多问题的出现，像ros2的多机通讯话题通讯异常，多传感器时间不同步导致的问题。

例如这里就以viobot2 和baton-mini进行多机通讯为例：

1. 确保两边的 ROS_DOMAIN_ID一致

viobot2 修改vim /home/PRR/.bashrc
```
...
```
或者在上位机上修改：
![alt text](image/image_ui-set-demoin-id.png)

修改完成后source一下，然后重启系统或者在终端重新launch启动节点就可以在viobot2上看到mini的话题了：
![alt text](image/image-show-ros2-topic.png)

(请不要在意这里的话题不是mini的话题，我这里是拿了两台viobot2来配置ntp)
