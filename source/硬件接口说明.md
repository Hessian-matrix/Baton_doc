...
如果是单纯串口通信的话就是接GND、TX、RX即可，需要注意的是串口电平是3.3V的如果接了5V会把串口烧掉。如果是直接供电的话可以接上5V的供电线。TRIG线可用做外部传感器触发信号线（只能做输出触发外部传感器）。

详细的串口通信协议和教程参考[SDK_Demo/Uart_Demo]([Uart_Demo — baton_doc 0.0 documentation](https://baton-doc.readthedocs.io/en/baton_mini/SDK_Demo/Uart_Demo.html))。
